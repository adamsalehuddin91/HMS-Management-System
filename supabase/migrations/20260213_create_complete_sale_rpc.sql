-- Migration: Create RPC function for atomic sale completion
-- This wraps all sale operations in a single transaction to prevent:
-- 1. Stock race conditions (overselling)
-- 2. Orphaned records if any operation fails
...
COMMENT ON FUNCTION public.complete_sale IS 'Atomically completes a sale with all related operations in a single transaction';
